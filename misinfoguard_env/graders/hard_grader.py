"""
Hard-difficulty grader for MisinfoGuard-Env.
Scores agent performance on high-density, high-spreader-strength scenarios.
"""

def grade(trajectory: dict) -> float:
    """
    Grade agent performance on hard task.
    ...
    # False reach component: lower is better (hard task needs strong containment)
    false_reach_score = 1.0 - final_false_reach
    
    # Recall component: higher is better
    recall_score = final_recall
    
    # Precision component: higher is better (hard task requires high precision)
    precision_score = final_precision
    
    # Weighted average (hard task emphasizes precision and false_reach containment)
    score = (
        0.20 * reward_score +
        0.35 * false_reach_score +
        0.20 * recall_score +
