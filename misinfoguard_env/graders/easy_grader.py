"""
Easy-difficulty grader for MisinfoGuard-Env.
Scores agent performance on low-density, low-spreader-strength scenarios.
"""

def grade(trajectory: dict) -> float:
    """
    Grade agent performance on easy task.
    ...
    # Reward component: normalize to [0, 1] range (assuming typical range is -150 to 0)
    ...
    # Precision component: higher is better
    precision_score = final_precision
    
    # Weighted average (easy task has lower precision requirement)
    score = (
        0.30 * reward_score +
